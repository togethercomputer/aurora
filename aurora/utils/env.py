...
def get_aurora_env_vars() -> dict[str, str]:
    """Return common environment variables for all Ray actors.

    Includes:
    - AURORA_* variables (e.g. log level) from the current process
    ...
    env.update({k: os.environ[k] for k in _AURORA_ENV_KEYS if k in os.environ})
    return env
