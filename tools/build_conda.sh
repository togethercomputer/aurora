...
if [ "$MODE" = "1" ]; then
    ...
    ENV_NAME="${ENV_NAME:-aurora}"
    micromamba create -n "$ENV_NAME" python=3.12 uv -c conda-forge -y
...
if [ "$MODE" = "1" ]; then
    micromamba run -n "$ENV_NAME" pip install -e "${SGLANG_FOLDER_NAME}/python[all]"
    micromamba run -n "$ENV_NAME" uv pip install -e ".[dev]"

    echo "aurora environment setup complete!"
