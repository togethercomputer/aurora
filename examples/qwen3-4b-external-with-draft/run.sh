#!/bin/bash
# External online training with draft model — Qwen3-4B
#
# This script:
...
#   4. Waits for scratch draft model to be created by training
...
#
# GPU allocation (default: 3 GPUs, non-overlapping):
#   - GPUs 0-1: training (FSDP/DP)
#   - GPU 2: external sglang server (TP=1, Qwen3-4B fits on 1 GPU)
#
# Usage:
#   bash examples/qwen3-4b-external-with-draft/run.sh [EXTRA_ARGS...]
#
# Environment variables:
#   SGLANG_GPUS           - GPUs for standalone sglang server (default: 2)
#   SGLANG_PORT           - sglang server port (default: 30000)
#   CALLBACK_PORT         - Training callback server port (default: 18080)
#   TARGET_MODEL          - Target model path (default: Qwen/Qwen3-4B)
...
# Draft model: trained from scratch — sglang uses the scratch draft dir
OUTPUT_DIR="$ROOT_DIR/outputs/qwen3-4b-external-with-draft"
...
LOG_FILE="$LOG_DIR/qwen3_4b_external_with_draft_${TIMESTAMP}.log"
SGLANG_LOG="$LOG_DIR/qwen3_4b_sglang_with_draft_${TIMESTAMP}.log"
...
echo "=============================================="
echo "External Training + Draft - Qwen3-4B"
echo "=============================================="
...
echo "Draft Model: $DRAFT_MODEL (scratch, created by training)"
...
echo "  Send requests: bash examples/qwen3-4b-external-with-draft/send_requests.sh"
echo "=============================================="
wait "$training_pid"
training_exit=$?
